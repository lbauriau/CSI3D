...
def decimating_conquest(vertices, faces):
    ...
    while fifo_gate != []:
        ...
        print(f"    current gate tags {[v.tag for v in entry_gate.vertices]}")
        ...
        elif ((front_vertex.flag == Flag.Free) and (valence > 6)) or (front_vertex.flag == Flag.Conquered) or front_vertex.isOnTheBoundary():
            patch = treatNullPatch(fifo_gate, entry_gate, front_face, front_vertex, output)
            if i == 0:
                first_gate_third_index = patch.bounding_vertices[2].id
        else:
...
def treatNullPatch(fifo_gate, entry_gate, front_face, front_vertex, output):
